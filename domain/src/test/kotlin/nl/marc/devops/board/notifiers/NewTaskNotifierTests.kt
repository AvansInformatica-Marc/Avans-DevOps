...
class NewTaskNotifierTests {
    @Test
    fun `FR-2_9) Notifications should be sent to all associated users when a task is moved forward`() {
        // Arrange
    ...
    @Test
    fun `FR-2_9) Notifications should be sent to all associated users when a task is moved back`() {
        // Arrange
        val getUserByRoleService = mockk<GetUserByRoleService>()
        val users = UsersFixture.generateUsers(2).toSet()
        every { getUserByRoleService.getUsersByRole(any()) } returns users
